...
/// Filter_AVXHelper is a helper class that is resposible for gathering data from a IntermediateTable_AVX to AVX registers. Once initialized
/// it holdes an register containing the virtual indexes sufficent for gathering.
...
class Filter_AVXHelper<uint32_t> {
   ...
   public:
    ...
    /// Gathers 32bit-integer from 16 consecutive row in the baseAddress storage. Their exact position within the baseAddress storage is
    /// determined by rowIndex and columnIndex. \param baseAddress base address of the data storage \param rowIndex index of the first row
...
template <typename T>
IntermediateTable_AVX<T> *apply_filters_AVX(IntermediateTable_AVX<T> &table, std::vector<Filter::Filter<T, SIMD::AVX512> *> &filters) {
    // Validate filters
