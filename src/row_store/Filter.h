...
IntermediateTable<T, Variant, Alignment> *apply_filters_unified(IntermediateTable<T, Variant, Alignment> &table,
    ...
    while (*vectorIterBegin != *vectorIterEnd) {
        ...
        for (int j = 0; j < filters.size(); ++j) {
            // TODO implement stuff for SIMD::None
            filteringMask = filters[j]->match(vectorIterBegin->gather(filters[j]->index), filteringMask);
        }
        // copy tuples to result
        auto scalarIter = vectorResultIter->getScalarIterator();
        copyMaskedTupleN(result, *scalarIter, _mm512_mask2int(filteringMask));
...
//
template <typename T, SIMD Variant, int Alignment>
void copyMaskedTupleN(IntermediateTable<T, Variant, Alignment> *result, IntermediateIterator<T, Variant, Alignment> &scalarIterBegin,
                      uint16_t mask) {
    if (!mask) return;
