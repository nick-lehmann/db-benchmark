...
//    public:
//     Table(uint64_t numAttributes, uint64_t numRows, const T **initialData) : Tables::AVX::ITable<T>(numAttributes, numRows, initialData) {
...
//         return std::make_tuple(reconstructTable(projection), (uint64_t)sizeOfIndexStorage, (uint64_t)projection.size());
//     }
