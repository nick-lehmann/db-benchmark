...
void benchmark(std::vector<uint64_t> &projection, std::vector<Filters::Filter<T, Variant> *> &filters,
               const std::string &fileId, unsigned rows=10000, unsigned columns=50,
               unsigned increase=10000,unsigned steps=100) {
...
std::vector<Filters::Filter<T, Variant> *> genFilters(unsigned int filterNr){
    ...
    for (int i = 0; i < filterNr; i++) {
        ...
        switch (rFilter) {
            case 0:
                filters.push_back(
                    new Filters::NotEqual<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
            case 1:
                filters.push_back(
                    new Filters::Equal<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
            case 2:
                filters.push_back(
                    new Filters::GreaterEqual<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
            case 3:
                filters.push_back(
                    new Filters::GreaterThan<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
            case 4:
                filters.push_back(
                    new Filters::LessEqual<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
            case 5:
                filters.push_back(
                    new Filters::LessThan<T, Variant>(std::rand() % 50, std::rand() % 100));
                break;
        }
    }
    return filters;
...
int main(int argc, char **argv) {
    std::vector<uint64_t> projection{0, 1, 2, 3, 12};
