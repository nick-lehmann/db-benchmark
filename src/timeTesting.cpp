...
void benchmark(std::vector<uint64_t> &projection, std::vector<Filters::Filter<T, Variant> *> &filters, const std::string &fileId,
               unsigned rows = 10000, unsigned columns = 50, unsigned increase = 10000, unsigned steps = 100) {
        ...
        switch (rFilter) {
            case 0:
                ...
                /* case 1:
                    ...
                    break; */
            case 1:
                filters.push_back(new Filters::GreaterEqual<T, Variant>(i, std::rand() % 21 + 40));
                break;
            case 2:
                filters.push_back(new Filters::GreaterThan<T, Variant>(i, std::rand() % 21 + 40));
                break;
            case 3:
                filters.push_back(new Filters::LessEqual<T, Variant>(i, std::rand() % 21 + 40));
                break;
            case 4:
                filters.push_back(new Filters::LessThan<T, Variant>(i, std::rand() % 21 + 40));
                break;
        }
    }
    return filters;
...
void increasingRows(std::vector<uint64_t> &projection) {
    ...
    Benchmark::benchmarkRows<Type64, SIMD::AVX512_Strided>(0, projection, filters64_S, 10000, 50, false, 10000, 100, 0, 100, 42,
    ...
    Benchmark::benchmarkRows<Type32, SIMD::AVX512_Strided>(0, projection, filters32_S, 10000, 50, false, 10000, 100, 0, 100, 42,
    ...
    for (int i = 0; i < filterNr; i++) {
        ...
        switch (rFilter) {
            case 0:
                ...
                break;
            /*case 1:
                filters.push_back(new Filters::Equal<T, Variant>(i, std::rand() % 21 + 40));
                break;*/
            case 1:
                filters.push_back(new Filters::GreaterEqual<T, Variant>(i, std::rand() % 21 + 40));
                break;
