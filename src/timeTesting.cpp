...
void benchmark(std::vector<uint64_t> &projection, std::vector<Filters::Filter<T, Variant> *> &filters, const std::string &fileId) {
    std::string fileName = "../output_files/PSbenchmark_" + fileId + ".csv";
    Benchmark::benchmarkRows<T, Variant>(2, projection, filters, 100, 50, false, 100, 500, 0, 100, 42, fileName);
    fileName = "../output_files/CSbenchmark_" + fileId + ".csv";
    Benchmark::benchmarkRows<T, Variant>(1, projection, filters, 100, 50, false, 100, 500, 0, 100, 42, fileName);
...
void increasingRows(std::vector<uint64_t> &projection) {
    ...
    Benchmark::benchmarkRows<Type64, SIMD::AVX512_Strided>(0, projection, filters_strided, 100, 50, false, 100, 500, 0, 100, 42,
...
void benchmarkFilter(std::vector<uint64_t> &projection, std::vector<Filters::Filter<T, Variant> *> &filters, const std::string &fileId) {
    ...
    Benchmark::benchmarkRows<T, Variant>(2, projection, filters, 1000000, 50, false, 100, 1, 0, 100, 42, fileName);
    fileName = "../output_files/CSbenchmark_" + fileId +"_fNR_"+std::to_string(filters.size())+ ".csv";
    Benchmark::benchmarkRows<T, Variant>(1, projection, filters, 1000000, 50, false, 100, 1, 0, 100, 42, fileName);
    fileName = "../output_files/RSbenchmark_" + fileId +"_fNR_"+std::to_string(filters.size())+ ".csv";
    Benchmark::benchmarkRows<T, Variant>(0, projection, filters, 1000000, 50, false, 100, 1, 0, 100, 42, fileName);
}
...
int main(int argc, char **argv) {
    ...
    increaseFilterNumber<uint64_t,SIMD::None>(25,projection,"Scalar64");
