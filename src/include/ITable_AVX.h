...
namespace Tables::AVX {
    ...
    public:
        ...
        query_table(std::vector<uint64_t> &projection, std::vector<Filters::AVX::Filter<T> *> &filters) = 0;

        /// Queries the table and returns the amount of rows in the result.
        /// \param projection column indices to project
        /// \param filters filters to apply
        virtual uint64_t
        query_count(std::vector<uint64_t> &projection, std::vector<Filters::AVX::Filter<T> *> &filters) = 0;
